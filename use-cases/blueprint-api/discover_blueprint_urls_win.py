#!/usr/bin/env python3
"""
URL Discovery Script for Blueprint API Documentation (Windows Compatible)
...
def test_server_connection(base_url: str = "http://localhost:8081"):
    ...
    try:
        ...
        print(f"SUCCESS: Server is accessible (Status: {response.status_code})")
        return True
...
def main():
    ...
    if not test_server_connection(base_url):
        print("\nERROR: Please start the HTTP server first, then run this script again")
        return
    ...
    if urls_file:
        print(f"\nSUCCESS: URL discovery completed!")
